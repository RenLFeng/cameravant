...
const http = axios.create({
  ...
  baseURL: 'http://wx.isafeiot.com/server', // 这里是放接口的统一前缀
...
vueAjax.prototype.setParams = function (type, api, params) {
  ...
  if (thisParams.data) {
    ...
    // thisParams.axiosParam[dataName] =(type === 'post' ? qs.stringify(thisParams.data) : thisParams.data);
    thisParams.axiosParam[dataName] =thisParams.data;
  }
  ...
  http(thisParams.axiosParam).then(res => {
    ...
    if (err.response) {
      ...
      if (response.status === 401) {
        ...
        Toast((thisParams.errorText || '网络请求错误') + '：' + err.response.status);
      }
      reject('网络请求错误：' + err.response.status);
    } else if (err.message === 'Network Error') {
      // 跨域或者其他网络错误
      if (thisParams.showError) {
        Toast((thisParams.errorText || '网络请求错误') + '：500');
      }
      reject('网络请求错误：500');
    } else {
